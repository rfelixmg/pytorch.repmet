...
class ResNetEncoder(nn.Module):
    def __init__(self, emb_dim, fc_dim=None, norm=True, pretrained=False):
        ...
        if self.pretrained:
            t = list(resnet18(pretrained=True).children())
            self.backbone = nn.Sequential(*list(resnet18(pretrained=True).children())[:-1])

            self.fc1 = nn.Linear(512, fc_dim)
...
class InceptionEncoder(nn.Module):
    def __init__(self, emb_dim):
        ...
        if self.fc_dim:
            # self.backbone = resnet18(pretrained=False, num_classes=self.fc_dim)
            self.bn1 = nn.BatchNorm1d(self.fc_dim)
            self.fc1 = nn.Linear(self.fc_dim, emb_dim)
